...
3. Adjust `Letter spacing`, `Space width`, or a selected glyph's advance override if testing spacing behavior.
4. Review the `Ready to export` diagnostics for non-blocking metrics warnings.
5. Click `Generate font`.
6. Open the downloaded ZIP.
7. Confirm it contains the selected `fonts/*.{otf,ttf,woff,woff2}` files for each valid board weight plus `index.html`.
8. Open `index.html` in a browser.
9. Confirm it shows one specimen row per generated weight.
10. Confirm representative expanded-catalog glyphs from uppercase, lowercase, numbers, punctuation, symbols, currency, math, marks, and Latin extended sections render with the exported outlines.
11. Confirm spacing in the test page matches the generated settings.

## Expected Result

- The exported ZIP includes selected OTF, TTF, WOFF, and WOFF2 files per verified generated weight in the current session.
- The ZIP `index.html` includes inline `@font-face` CSS for the selected packaged formats and one row per generated weight.
- The ZIP page opens without extra setup after extraction.
- Supported expanded-catalog glyphs use the Typegen font when available.
...
- OTF remains Typegen's native generated font; TTF, WOFF, and WOFF2 are converted package outputs.
- WOFF2 uses a bundled encoder and may take longer than OTF-only export.
